...
er_meta <- read.csv("/Users/emb016/Documents/HBC04292_Hazra/ER_metadata.csv")

full3 <- full2 %>% left_join(zipdata, by=c("cc_study_id.y" = "Study_ID")) 
full3 <- full3 %>% left_join(er_meta, by=c("cc_study_id.y" = "ID_FINAL","Zipcode"="zip", "Dx_FUtime.x"="dx_futime"))
...
info <- data.frame(mcols(dds))
# Get the design matrix from the dds object that already ran
design_matrix <- model.matrix(design(dds), colData(dds))

# Now calculate Cook's cutoff
cooks_cutoff <- qf(0.99, ncol(dds), ncol(dds) - ncol(design_matrix))

# Export to a data frame for easier manipulation
cooks_df <- as.data.frame(cooks_distances)
cooks_df$gene <- rownames(cooks_df)
## KRT10


KRT10_cooks <- cooks_distances["ENSG00000186395", ]
KRT10_cooks[KRT10_cooks > 1.495]
result <- (norm_c["ENSG00000186395",c("wu127","wu194","wu198","wu227","wu260","wu282")])


## CA2

CA2_cooks <- cooks_distances["ENSG00000104267", ]
CA2_cooks[CA2_cooks > 1.495]
result2 <- (norm_c["ENSG00000104267",c("wu148","wu270")])

# RAFGAL1


RAF_cooks <- cooks_distances["ENSG00000174373", ]
RAF_cooks[RAF_cooks > 1.495]


# BACH2


BACH_cooks <- cooks_distances["ENSG00000112182", ]
BACH_cooks[BACH_cooks > 1.495]
...
to_graph$loc <- metadata2$loc
to_graph$endocrine <- metadata2$Endocrine_Therapy.x
to_graph$ER <- metadata2$ER_composite
to_graph2 <- to_graph %>% drop_na(ER)

to_graph$endocrine <- gsub("0","No Endocrine Therapy", to_graph$endocrine)
to_graph$endocrine <- gsub("1","Endocrine Therapy", to_graph$endocrine)
to_graph2$ER <- gsub("0","ER -", to_graph2$ER)
to_graph2$ER <- gsub("1","ER +", to_graph2$ER)
...
for (i in seq(1)) {
  ...
  ggsave(file = paste0("/Users/emb016/Documents/HBC04292_Hazra/full_data/coi_interaction_",names[[i]],".png"), out[[i]])  ## save plot
    ...
    out2[[i]] <- ggplot(to_graph,aes(x=endocrine,y=temp, color=as.factor(endocrine)))  + geom_point()+ geom_boxplot() + xlab("Endocrine Therapy Status") + ylab("ESR1") + theme(legend.position = "none")
    
  ggsave(file = paste0("/Users/emb016/Documents/HBC04292_Hazra/full_data/Endo_therapy_",names[[i]],".png"), out2[[i]]) 
  
    out3[[i]] <- ggplot(to_graph2,aes(x=ER,y=temp, color=as.factor(ER)))  + geom_point()+ geom_boxplot() + xlab("Endocrine Receptor") + ylab("ESR1") + theme(legend.position = "none")
    
      ggsave(file = paste0("/Users/emb016/Documents/HBC04292_Hazra/full_data/ER_status_",names[[i]],".png"), out3[[i]]) 
}
