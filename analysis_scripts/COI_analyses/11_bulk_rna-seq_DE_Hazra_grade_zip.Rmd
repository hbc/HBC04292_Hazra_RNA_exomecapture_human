...
#write out results
 write.table(sig_genes_coi, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/sig_genes_coi_caseinteraction_p0.1.csv", sep=",", row.names = FALSE)
 write.table(res_tb_coi, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/all_results_coi_caseinteraction_p0.1.csv", sep=",", row.names = FALSE)
write.table(sig_genes_int2, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/sig_genes_caseinteraction_p0.1.csv", sep=",", row.names = FALSE)
 write.table(res_tb_int2, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/all_results_caseinteraction_p0.1.csv", sep=",", row.names = FALSE) 
...
for (i in seq(1,4)) {
  ...
  out2[[i]] <- ggplot(to_graph,aes(x=coi,y=temp))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") + theme(legend.position = "NONE") + ylim(0,5000)
...
out <- vector("list", length = 38)
out2 <- vector("list", length = 38)
...
for (i in seq(1,38)) {
  ...
  out[[i]] <- ggplot(to_graph,aes(x=coi,y=temp, color=as.factor(casecontrol)))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") + xlab("COI")

    out2[[i]] <- ggplot(to_graph,aes(x=coi,y=temp, color=as.factor(casecontrol)))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") + facet_wrap(~race) + xlab("COI")
...
# DESEQ-   gene expression ~ Intronic Rate + Percent GC + Race  + Dx_FUtime +  Age + Grade + coi + coi*race.   - M2
...
dds2 <- DESeqDataSetFromMatrix(raw_counts4, 
                              colData = metadata2, 
                              design = ~  scaled_age + scaled_FU + scaled_gc + scaled_intronic + Race.x + Grade.x + scaled_z_coi + scaled_z_coi*Race.x)
...
There were no significant differentially expressed genes identified. 

### COI X Race

Using a Wald test, we tested for an interaction between COI and case control status using an adjusted p-value cut-off of 0.1. 

There were 3 significantly DE genes identified.


```{r}

## get normalized counts
norm_c <- data.frame(counts(dds2, normalized=TRUE))
norm_c$gene <- rownames(norm_c)

shrunken_res_coi <- data.frame(results(dds2,
         name="scaled_z_coi"))



shrunken_res_int <- data.frame(results(dds2,
         name="Race.x2.scaled_z_coi"))




res_tb_coi2 <- shrunken_res_coi %>%
  data.frame() %>%
  rownames_to_column(var="gene") %>% 
  as_tibble() %>% 
  left_join(pruned_annotations, 
...
res_tb_coi2 <- res_tb_coi2[res_tb_coi2$baseMean>0,]



res_tb_int <- shrunken_res_int %>%
  data.frame() %>%
  rownames_to_column(var="gene") %>% 
  as_tibble() %>% 
  left_join(pruned_annotations, 
...
res_tb_int <- res_tb_int[res_tb_int$baseMean>0,]
...
sig_genes_coi2 <- res_tb_coi2 %>%
        dplyr::filter(padj < 0.1) %>%
        arrange(padj)


sig_genes_int <- res_tb_int %>%
        dplyr::filter(padj < 0.1) %>%
        arrange(padj)
...
#write out results
 write.table(sig_genes_coi2, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/sig_genes_coi_raceinteraction_p0.1.csv", sep=",", row.names = FALSE)
 write.table(res_tb_coi2, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/all_results_coi_raceinteraction_p0.1.csv", sep=",", row.names = FALSE)
 
  write.table(sig_genes_int, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/sig_genes_raceinteraction_p0.1.csv", sep=",", row.names = FALSE)
 write.table(res_tb_int, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/all_results_raceinteraction_p0.1.csv", sep=",", row.names = FALSE)
# 
# write.table(sig_genes_FU, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/sig_genes_dxfutime_p0.1.csv", sep=",", row.names = FALSE)
# write.table(res_tb_FU, file="/Users/emb016/Documents/HBC04292_Hazra/full_data/Results/all_results_dxfutime_p0.1.csv", sep=",", row.names = FALSE)
# 
...
### Top genes with Interaction WITH RACE

Here, we've plotted the abundance (DESeq2-normalized counts) of the top 20 differentially expressed genes (DEGs, ranked by adjusted p-value) in each sample. We plot COI vs. abundance and add a trendline for the top 3 genes.  Some of these look convincing but others do not.

**We plot this 3x, once zoomed in, and once separated by case control**
...
rna = sig_genes_int$gene
names = sig_genes_int$external_gene_name
...
for (i in seq(1,3)) {
  ...
  out[[i]] <- ggplot(to_graph,aes(x=coi,y=temp, color=as.factor(race)))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") 
  
    out2[[i]] <- ggplot(to_graph,aes(x=coi,y=temp, color=as.factor(race)))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") + ylim(0,1000)
    
      out3[[i]] <- ggplot(to_graph,aes(x=coi,y=temp, color=as.factor(race)))  + geom_point()+ ylab(paste0(names[[i]])) + geom_smooth(method = "lm") + facet_wrap(~casecontrol)
