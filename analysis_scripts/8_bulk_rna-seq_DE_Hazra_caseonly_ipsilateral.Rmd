...
## Subset only ipsilateral cases


```{r}
meta_case <- subset(met, met$CaseControl==1 & meta$Recur_Loc !=1)
...
rownames(to_use) <- to_use$sample
colnames(to_use) <- gsub(".x","", colnames(to_use))

colnames(to_use)[8] <- "Age_DCIS_Dx"
colnames(to_use)[9] <- "Dx_FUtime"
colnames(to_use)[2] <- "Percent_GC"
colnames(to_use)[4] <- "Intergenic_Rate"
colnames(to_use)[5] <- "Intronic_Rate"
colnames(to_use)[6] <- "Exonic_Rate"
colnames(to_use)[7] <- "Total_Reads"
colnames(to_use)[3] <- "rRNA_Rate"
colnames(to_use)[12] <- "SBE"
...
pdf(file = "/Users/emb016/Documents/HBC04292_Hazra/full_data/FigSX_covariate_SBE_Model.pdf",   # The directory you want to save the file in
...
meta_case$scaled_age <- scale(meta_case$Age_DCIS_Dx)


meta_case$Inv_Dx <- as.factor(meta_case$Inv_Dx)
meta_case$Menopause <- as.factor(meta_case$Menopause)
...
# DESEQ -   gene expression ~ Menopause + Percent GC + Age + Inv_dx.   - M1
...
dds <- DESeqDataSetFromMatrix(counts_case, 
                              colData = metadata2, 
                              design = ~   scaled_gc + Menopause +  scaled_age +  Inv_Dx)
...
shrunken_res_Meno <- data.frame(results(dds,
         name="Menopause_1_vs_0"))
...
Here, we've plotted the abundance (DESeq2-normalized counts) of the top 3 differentially expressed genes (DEGs, ranked by adjusted p-value) from the analysis with both ipsilateral and contralateral samples using the ipsilateral only data. 
...
rna = c("ENSG00000188868","ENSG00000221823","ENSG00000171428")
names = c("ZNF563","PPP3R1","NAT1")

out <- vector("list", length = 3)

to_graph = data.frame(to_test[,rna])
to_graph$Inv_Dx <- metadata2$Inv_Dx
to_graph$Inv_Dx <- gsub("0", "No", to_graph$Inv_Dx)
to_graph$Inv_Dx <- gsub("1", "Yes", to_graph$Inv_Dx)
...
for (i in seq(1,3)) {
  ...
  out[[i]] <- ggplot(to_graph,aes(x=Inv_Dx,y=temp, color=Inv_Dx)) + geom_boxplot() + geom_point() + ylab(paste0(names[[i]])) + theme(legend.position = "NONE") + xlab("Invasive Diagnosis") + scale_color_manual(values=c("#E5F5F9","#2CA25F"))
...
grid.arrange(grobs = out, ncol = 1) ## display plot
ggsave(file = "/Users/emb016/Documents/HBC04292_Hazra/full_data/fig2_invdx_nocontra.png", arrangeGrob(grobs = out, ncol = 1))  ## save plot
